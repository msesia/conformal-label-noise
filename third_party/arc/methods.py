...
class SplitConformal:
    def __init__(self, X, Y, black_box, K, alpha, n_cal=0.5, random_state=2020, allow_empty=True, 
                 label_conditional=False, calibration_conditional=False, gamma=None, pre_trained=False, verbose=False):
        self.allow_empty = allow_empty
        self.K = K
        ...
        if label_conditional:
            alpha_correction = np.ones((self.K,))
            for k in range(self.K):
                idx_k = np.where(Y_calib==k)[0]
                n_k = len(idx_k)
                if n_k>0:
                    if self.calibration_conditional:
                        cc_correction = np.sqrt(np.log(1.0/self.gamma)/(2.0*n_k))
                        level_adjusted = (1.0-alpha+cc_correction)*(1.0+1.0/float(n_k))
                    else:
                        level_adjusted = (1.0-alpha)*(1.0+1.0/float(n_k))
                    alpha_correction[k] = mquantiles(scores[idx_k], prob=level_adjusted)
        else:
    ...
    def predict(self, X, random_state=2020):
        ...
        if self.label_conditional:
            ...
            for k in range(self.K):
                alpha_k = self.alpha_calibrated[k]
                S_k = grey_box.predict_sets(alpha_k, epsilon=epsilon, allow_empty=self.allow_empty)
                for i in range(n):
                    if k in S_k[i]:
